...
export class DataStore<T> {
  ...
  private networkDelay =
    AppShellConfig.settings && AppShellConfig.settings.networkDelay
      ? AppShellConfig.settings.networkDelay
      : 0;
  ...
  public static AppendShell<T>(
    dataObservable: Observable<T>,
    shellModel: T,
    networkDelay = 400
  ): Observable<T & ShellModel> {
    ...
    return combineLatest([delayObservable, dataObservable]).pipe(
      ...
      map(([delayValue, dataValue]: [boolean, T]): T & ShellModel =>
        Object.assign(dataValue, { isShell: false })
      ),
  ...
  load(dataSourceObservable: Observable<T>): void {
    ...
    dataSourceWithShellObservable.subscribe((dataValue: T & ShellModel) => {
